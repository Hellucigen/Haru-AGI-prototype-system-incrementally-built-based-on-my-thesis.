...
class ActionExecutor:
    def __init__(self, graph: KnowledgeGraph, activation_manager, threshold: float = 0.2):
        self.graph = graph
    ...
    def execute_pending_actions(self, current_focus: str = None, current_text: str = ""):
        ...
        for act in actions:
            ...
            exec_context = {
                ...
                "time": time,
                "Node": Node,
                "__builtins__": {'max': max, 'min': min, 'len': len, 'int': int, 'float': float, 'str': str, 'dict': dict, 'list': list, 'set': set}
            }
            try:
